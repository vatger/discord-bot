    ...
    async run(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const sent = await interaction.followUp({
                embeds: [warningEmbed('Loading', null, `Loading METAR for ${icao?.toUpperCase() ?? 'N/A'}, please wait.`)],
                fetchReply: true,
                ephemeral: true,
            });
            ...
            if (icao == null) {
                await interaction.followUp({
                    embeds: [dangerEmbed('METAR Failed', null, 'Failed to resolve ICAO: null provided')],
                    ephemeral: true,
                });
                return;
            }
            ...
            try {
                ...
                await interaction.editReply({
                    embeds: [dangerEmbed('METAR Failed', null, `Error loading METAR for ${icao.toUpperCase()}`)],
                });
            }
        } catch (e: any) {
            await interaction.followUp({
    ...
    build(): any {
        return new SlashCommandBuilder()
            ...
            .addStringOption(option =>
                option.setName('icao').setDescription('ICAO of the requested aerodrome').setRequired(true).setMaxLength(4).setMinLength(4),
            );
    }
}
