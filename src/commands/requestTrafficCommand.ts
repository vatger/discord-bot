    ...
    async run(interaction: ChatInputCommandInteraction) {
        try {
            ...
            const sent = await interaction.followUp({
                embeds: [warningEmbed('Working ...', null, `Building the request Post for ${icao?.toUpperCase() ?? 'N/A'}, please wait.`)],
                fetchReply: true,
                ephemeral: true,
            });
            ...
            if (icao == null) {
                await interaction.followUp({
                    embeds: [dangerEmbed('Building Failed', null, 'Failed to resolve ICAO Code')],
                    ephemeral: true,
                });
                return;
            }
            ...
            try {
                let res: AxiosResponse = await axios.get(`https://avwx.rest/api/metar/${icao}?format=json&onfail=error`, {
                    headers: {
                ...
                const requestEmbed: EmbedBuilder = new EmbedBuilder()
                    ...
                    .setDescription(`ATC service provided by <@${interaction.user.id}>`)
                    .addFields({
                        name: 'METAR',
                        value: '```' + res?.data.raw + '```',
                    })
                ...
                await interaction.editReply({
                    embeds: [dangerEmbed('Building failed', null, `Error creating request for ${icao.toUpperCase()}`)],
                });
            }
        } catch (e: any) {
            await interaction.followUp({
    ...
    build(): any {
        return new SlashCommandBuilder()
            ...
            .addStringOption(option =>
                option.setName('icao').setDescription('ICAO of the aerodrome').setRequired(true).setMaxLength(4).setMinLength(4),
            )
            .addStringOption(option => option.setName('rwys').setDescription('Set the runway(s) in use.').setRequired(false))
            .addStringOption(option =>
                option
                    .setName('vfrcondition')
                    .setDescription('Set the VFR condition')
                    .setRequired(false)
                    .setChoices({ name: 'Good', value: 'good' }, { name: 'Medium', value: 'medium' }, { name: 'Bad', value: 'bad' }),
            )
            .addStringOption(option =>
                option
                    .setName('ifrcondition')
                    .setDescription('Set the IFR condition')
                    .setRequired(false)
                    .setChoices({ name: 'Good', value: 'good' }, { name: 'Medium', value: 'medium' }, { name: 'Bad', value: 'bad' }),
            )
            .addStringOption(option => option.setName('remark').setDescription('Set any freetext you like.').setRequired(false));
    }
}
